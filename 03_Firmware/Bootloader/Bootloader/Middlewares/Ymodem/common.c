...
uint32_t SerialKeyPressed(uint8_t *key)
{
  ...
  //   return 0;
  // }
  /* 非阻塞查询：有数据读走并返回 1，无数据立即返回 0。
     不能阻塞，否则 Ymodem 发不出起始的 'C'，PC 端不会开始发送 */
  if (USART_GetFlagStatus(USART1, USART_FLAG_RXNE) != RESET)
  {
...
#if 0  /* F1 遗留：F4 无 option byte API（OB_IWDG_SW 等），编译不过，暂禁用 */
/**
...
#if 0  /* F1 遗留：菜单 + 写保护检测，用到 F1 API 且依赖未实现的 SerialDownload/SerialUpload */
/**
